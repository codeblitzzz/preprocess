#!/usr/bin/env python3
"""
Generate embeddings for JSON chunks using Ollama Qwen model and store in ChromaDB.

Requirements:
    pip install chromadb requests numpy

Usage:
    python generate_embeddings_ollama.py --chunks-dir "./json_chunks" --collection-name "attribute"
"""
...
class ChromaDBManager:
    ...
    def __init__(self, db_path: str = "./chroma_db", collection_name: str = "attribute"):
        ...
        try:
            ...
            self.collection = self.client.create_collection(
                ...
                metadata={"description": "Healthcare regulatory document chunks with Qwen embeddings"}
            )
...
def load_json_chunks(chunks_dir: Path) -> List[Dict[str, Any]]:
...
def prepare_metadata_for_chroma(chunk_data: Dict[str, Any]) -> Dict[str, Any]:
    """Prepare metadata for ChromaDB (only string, int, float, bool values)."""
    metadata = {
        # Core identifiers
        "chunk_id": chunk_data["chunk_id"],
        "source_file": chunk_data["source_file"],
        "source_row": chunk_data["source_row"],
        "chunk_index": chunk_data["chunk_index"],
        "total_chunks_from_row": chunk_data["total_chunks_from_row"],
        
        # Content metadata
        "title": chunk_data["content"]["title"],
        "token_count": chunk_data["metadata"]["token_count"],
        "character_count": chunk_data["metadata"]["character_count"],
        "created_at": chunk_data["metadata"]["created_at"],
        
        # Embedding metadata
        "semantic_type": chunk_data["embedding_metadata"]["semantic_type"],
        "domain": chunk_data["embedding_metadata"]["domain"],
        "embedding_model": "qwen3-embedding:0.6b",
        
        # Attributes as individual fields for better filtering
        "has_medicaid_timely_filing": "Medicaid Timely Filing" in chunk_data["metadata"]["attributes"],
        "has_medicare_timely_filing": "Medicare Timely Filing" in chunk_data["metadata"]["attributes"],
        "has_claims_processing": "Claims Processing" in chunk_data["metadata"]["attributes"],
        "has_provider_requirements": "Provider Requirements" in chunk_data["metadata"]["attributes"],
        "has_regulatory": "Regulatory" in chunk_data["metadata"]["attributes"],
        "has_reimbursement": "Reimbursement" in chunk_data["metadata"]["attributes"],
        "has_fee_schedule": any("Fee Schedule" in attr for attr in chunk_data["metadata"]["attributes"]),
        
        # Primary attribute (first one)
        "primary_attribute": chunk_data["metadata"]["attributes"][0] if chunk_data["metadata"]["attributes"] else "General",
        
        # Source columns info
        "num_source_columns": len(chunk_data["metadata"]["source_columns"]),
        
        # Key fields availability
        "has_description": "description" in chunk_data["content"]["key_fields"],
        "has_example_language": "example_extracted_language" in chunk_data["content"]["key_fields"],
        "has_example_section": "example_section_in_document" in chunk_data["content"]["key_fields"]
    }
    
    return metadata

def process_chunks_to_embeddings(chunks_dir: Path, 
                                collection_name: str = "attribute",
                                db_path: str = "./chroma_db_qwen",
                                model_name: str = "qwen3-embedding:0.6b",
                                batch_size: int = 5) -> None:
    """Main function to process JSON chunks and generate embeddings using Ollama."""
    ...
    for i in range(0, len(chunks), batch_size):
        ...
        for chunk_data in batch_chunks:
            ...
            text_for_embedding = chunk_data["embedding_metadata"]["text_for_embedding"]
            ...
            # Prepare metadata
            metadata = prepare_metadata_for_chroma(chunk_data)
    ...
    # Test query
    logger.info("Testing similarity search with Qwen embeddings...")
    if chunks:
        test_text = "medicaid timely filing requirements"
        ...
        if test_embedding:
            ...
            if results.get('documents') and results['documents'][0]:
                logger.info("Top result titles:")
                for i, metadata in enumerate(results['metadatas'][0][:3]):
                    logger.info(f"  {i+1}. {metadata.get('title', 'N/A')}")
    
    logger.info("✅ Qwen embedding generation completed successfully!")

def main():
    parser = argparse.ArgumentParser(description="Generate embeddings for JSON chunks using Ollama Qwen model")
    parser.add_argument("--chunks-dir", type=str, default="./json_chunks", help="Directory containing JSON chunk files")
    parser.add_argument("--collection-name", type=str, default="attribute", help="ChromaDB collection name")
